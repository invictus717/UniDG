...
class IIDAccuracySelectionMethod_Adaption(SelectionMethod):
    """Picks argmax(mean(env_out_acc for env in train_envs))"""
    name = "online training-domain validation set"
    ...
    def _step_acc(self, record):
        ...
        for i in itertools.count():
            if f'env{i}_in_acc' not in record:
                break
            if i != test_env:
                val_env_keys.append(f'env{i}_in_acc')
        test_in_acc_key = 'env{}_in_acc'.format(test_env)
        return {
            'val_acc': np.mean([record[key] for key in val_env_keys]),
            'test_acc': np.max(record[test_in_acc_key])
        }
    ...
    def run_acc(self, run_records):
        ...
        if not len(test_records):
            return None
        return test_records.map(self._step_acc).argmax('test_acc')
